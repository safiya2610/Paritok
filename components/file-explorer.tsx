      ...
      .map((node) => {
        ...
        return (
          <div key={node.path} className="select-none">
            <div
              className={cn(
                "flex items-center py-1.5 px-2 hover:bg-muted rounded cursor-pointer text-sm text-foreground hover:text-primary transition-colors",
                level > 0 && "ml-4",
              )}
  ...
  return (
    <div className="flex flex-col h-full bg-card text-foreground">
      <div className="p-3 border-b border-border bg-card">
        <div className="text-lg mb-3 flex items-center gap-1 font-mono">
          <Link href={`/${username}`} className="text-muted-foreground hover:text-foreground transition-colors font-medium">
            {username}
          </Link>
          <span className="text-muted-foreground">/</span>
          <Link href={`https://github.com/${username}/${repo}`} className="text-foreground hover:text-primary font-semibold transition-colors">
            {repo}
          </Link>
        </div>
        <div className="relative group">
          <Search className="h-3.5 w-3.5 absolute left-2.5 top-2.5 text-muted-foreground group-focus-within:text-foreground transition-colors" />
          <Input
            placeholder="Search files..."
            className="pl-8 h-8 bg-background border-border text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:ring-1 focus:ring-primary shadow-sm"
            value={searchQuery}
