...
export default function FileViewer({ repoData }: FileViewerProps) {
  ...
  if (isLoading) {
    return (
      <div className="p-4">
        <div className="border-b border-border bg-card p-2 px-4 text-sm font-mono text-muted-foreground rounded-t-lg">{filePath}</div>
        <div className="p-4">
      ...
      case 'image':
        return (
          <div className="flex flex-col h-full">
            <div className="flex items-center justify-end gap-2 p-2 border-b border-border bg-card">
              <Button variant="ghost" size="icon" onClick={handleZoomOut} disabled={zoomLevel <= 0.5}>
                <ZoomOut className="h-4 w-4 text-foreground" />
              </Button>
              <span className="text-xs font-mono w-12 text-center text-foreground">{Math.round(zoomLevel * 100)}%</span>
  ...
  return (
    <div className="flex flex-col h-full bg-background">
      <div className="border-b border-border bg-card p-2 px-4 text-sm font-mono text-muted-foreground rounded-t-lg flex justify-between items-center">
        <span className="font-semibold text-foreground">{filePath}</span>
        <span className="text-xs opacity-70 uppercase text-primary font-bold">{fileType}</span>
      </div>
