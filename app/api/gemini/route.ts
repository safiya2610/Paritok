  ...
  for (const { name: clientName, client } of clients) {
    for (const modelName of FALLBACK_MODELS) {
      try {
        ...
        logger.info(`Generated response using ${clientName} with model: ${modelName}`, { prefix: 'Gemini' });
        return result.response.text();
      } catch (error) {
        lastErrorMsg = error instanceof Error ? error.message : String(error);
        logger.warn(`Failed with ${clientName} and model ${modelName}: ${lastErrorMsg}`, { prefix: 'Gemini' });
        // Continue to the next model
      }
    }
  }

  throw new Error(`All models and API keys failed. Last error: ${lastErrorMsg}`);
}
...
export async function POST(req: Request) {
  try {
    ...
    const { username, repo, query, filePath, fetchOnlyCurrentFile = false, history = [], useParitok = true } = await req.json();
    ...
    if (filePath && fetchOnlyCurrentFile) {
      ...
      if (useParitok) {
        const optimized = await optimizeContext(fileContent, query);
        finalFileContent = optimized.compressed;
...
Provide a detailed, technical response that directly addresses the user's query about this specific file.`;
      ...
      contextStats.totalChars = finalFileContent.length;
    } else {
      ...
      try {
        ...
        try {
          ...
          if (repoData) {
            ...
            // Optimize context with Paritok if enabled
            let finalContent = repoData.content;
            if (useParitok) {
               const optimized = await optimizeContext(repoData.content, query);
               finalContent = optimized.compressed;
               paritokMetrics = optimized;
            }
            ...
            contextStats.totalChars = finalContent.length;
        ...
        if (!repoData) {
          ...
          if (gitIngestData && !gitIngestData.error) {
            ...
            let finalContent = gitIngestData.content;
            
            if (useParitok) {
               const optimized = await optimizeContext(gitIngestData.content, query);
               finalContent = optimized.compressed;
               paritokMetrics = optimized;
            }
